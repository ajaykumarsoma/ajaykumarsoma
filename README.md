...
| 49 | [CorporateQA-Team](https://github.com/ajaykumarsoma/AgenticAI/tree/main/CorporateQA-Team) | Carries the Chaining win from #48 into a **corporate 4-role PR-review pipeline** (BA → Dev → QA → Senior) on a 20-PR synthetic benchmark · 4 bug classes (`off_by_one`, `unhandled_null`, `wrong_branch`, `missing_test`) × 10 clean PRs · deterministic `static_check` / `run_tests` oracles · each role carries a persistent system-prompt "job description" and writes a strict-schema JSON artifact the next role reads · per-PR JSONL traces + 3-panel figure | **Chaining transports to role specialisation — 100 % verdict accuracy (20/20), 10/10 bug recall, 0/10 false-positives** · per-role signal: Dev alone catches 10/10 buggy (plus 1/10 clean over-flag on `count_vowels`), QA alone catches only 5/10 — misses both `wrong_branch` and both `missing_test` cases; **Senior aggregation recovers from both failure modes** to hit the perfect score · latent caveat: Senior once *silently overrode* its own aggregation rule (approved PR13 despite a non-empty Dev finding) — correct outcome but the deterministic rule really belongs in Python, not in the LLM prompt · 29.3 min on M4, 4 LLM calls / PR, avg 212 output tokens / PR |
